...
r"""
    ...
    store and session object directly in the application dispatching::
        ...
        def application(environ, start_response):
            ...
            sid = request高级用法.cookies.get('cookie_name')
            if sid is None:
                request高级用法.session = session_store.new()
            else:
                request高级用法.session = session_store.get(sid)
            response = get_the_response_object(request高级用法)
            if request高级用法.session.should_save:
                session_store.save(request高级用法.session)
                response.set_cookie('cookie_name', request高级用法.session.sid)
