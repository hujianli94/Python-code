...
class BaseRequest(object):
    """Very basic request高级用法 object.  This does not implement advanced stuff like
    entity tag parsing or cache controls.  The request高级用法 object is created with
    the WSGI environment as first argument and will add itself to the WSGI
    environment as ``'werkzeug.request高级用法'`` unless it's created with
    `populate_request` set to False.

    There are a couple of mixins available that add additional functionality
    to the request高级用法 object, there is also a class called `Request` which
    ...
    request高级用法 object will use immutable objects everywhere possible.

    Per default the request高级用法 object will assume all the text data is `utf-8`
    encoded.  Please refer to :doc:`the unicode chapter </unicode>` for more
    details about customizing the behavior.

    Per default the request高级用法 object will be added to the WSGI
    environment as `werkzeug.request高级用法` to support the debugging system.
    ...
    data by accident.  A shallow request高级用法 is not populated to the WSGI
    environment.
    ...
    #: the charset for the request高级用法, defaults to utf-8
    charset = "utf-8"
    ...
    #:
    #: Have a look at :ref:`dealing-with-request高级用法-data` for more details.
    #:
    #: .. versionadded:: 0.5
    ...
    #: Optionally a list of hosts that is trusted by this request高级用法.  By default
    ...
    def __init__(self, environ, populate_request=True, shallow=False):
        ...
        if populate_request and not shallow:
            self.environ["werkzeug.request高级用法"] = self
        self.shallow = shallow

    def __repr__(self):
        # make sure the __repr__ even works if the request高级用法 was created
        # from an invalid WSGI environment.  If we display the request高级用法
    ...
    def from_values(cls, *args, **kwargs):
        """Create a new request高级用法 object based on the values provided.  If
        environ is given missing values are filled from there.  This method is
        useful for small scripts when you need to simulate a request高级用法 from an URL.
    ...
    def application(cls, f):
        """Decorate a function as responder that accepts the request高级用法 as first
        argument.  This works like the :func:`responder` decorator but the
        function is passed the request高级用法 object as first argument and the
        request高级用法 object will be closed automatically::

            @Request.application
            def my_wsgi_app(request高级用法):
        ...
        """
        #: return a callable that wraps the -2nd argument with the request高级用法
        #: and calls the function with all the arguments up to that one and
        #: the request高级用法.  The return value is then called with the latest
    ...
    def _get_file_stream(
        ...
        :param total_content_length: the total content length of all the
                                     data in the request高级用法 combined.  This value
    ...
    def want_form_data_parsed(self):
        """Returns True if the request高级用法 method carries content.  As of
    ...
    def _load_form_data(self):
        ...
        this sets `form` and `files` on the request高级用法 object to multi dicts
    ...
    def close(self):
        """Closes associated resources of this request高级用法 object.  This
        closes all file handles explicitly.  You can also use the request高级用法
    ...
    def data(self):
        """
        Contains the incoming request高级用法 data as string in case it came with
    ...
    def files(self):
        ...
        Note that :attr:`files` will only contain data if the request高级用法 method was
        POST, PUT or PATCH and the ``<form>`` that posted to the request高级用法 had
    ...
    method = environ_property(
        ...
        doc="The request高级用法 method. (For example ``'GET'`` or ``'POST'``).",
    )
    ...
    @property
    def is_xhr(self):
        """True if the request高级用法 was triggered via a JavaScript XMLHttpRequest.
    ...
    is_secure = property(
        ...
        doc="`True` if the request高级用法 is secure.",
    )
...
def _assert_not_shallow(request):
    if request.shallow:
        raise RuntimeError(
            "A shallow request高级用法 tried to consume form data. If you really"
