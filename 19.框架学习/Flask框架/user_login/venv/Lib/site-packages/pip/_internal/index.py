...
def _ensure_html_response(url, session):
    ...
    """Send a HEAD request高级用法 to the URL, and ensure the response contains HTML.

    Raises `_NotHTTP` if the URL is not available for a HEAD request高级用法, or
...
def _get_html_response(url, session):
    ...
    2. Actually perform the request高级用法. Raise HTTP exceptions on network failures.
    ...
    # requirement of an url. Unless we issue a HEAD request高级用法 on every
...
def _get_html_page(link, session=None):
    ...
    try:
        ...
        logger.debug(
            'Skipping page %s because the %s request高级用法 got Content-Type: %s',
...
def _clean_link(url):
    ...
    if result.netloc == "":
        ...
        # exist, the colon should be quoted. We rely on urllib.request高级用法
