...
def get_debug_queries():
    ...
    to the database.  This information is available until the end of request高级用法
...
class BaseQuery(orm.Query):
    ...
    def paginate(self, page=None, per_page=None, error_out=True, max_per_page=None):
        ...
        the request高级用法 query. If ``max_per_page`` is specified, ``per_page`` will
        be limited to that value. If there is no request高级用法 or they aren't in the
...
class SQLAlchemy(object):
    ...
    def create_scoped_session(self, options=None):
        ...
        created and removed with the request高级用法/response cycle, and should be fine
        in most cases.
