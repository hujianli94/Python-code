...
.. doctest::
   ...
   >>> from xtbson import Binary, Code
   >>> from xtbson.json_util import dumps
   >>> dumps([{'foo': [1, 2]},
   ...        {'bar': {'hello': 'world'}},
...
.. doctest::
   ...
   >>> from xtbson.json_util import loads
...
It won't handle :class:`~xtbson.binary.Binary` and :class:`~xtbson.code.Code`
instances (as they are extended strings you can't provide custom defaults),
but it will be faster as there is less recursion.

.. versionchanged:: 2.3
   Added dumps and loads helpers to automatically handle conversion to and
   from json and supports :class:`~xtbson.binary.Binary` and
   :class:`~xtbson.code.Code`
...
.. versionchanged:: 1.8
   Added support for encoding/decoding :class:`~xtbson.max_key.MaxKey`
   and :class:`~xtbson.min_key.MinKey`, and for encoding
   :class:`~xtbson.timestamp.Timestamp`.
...
import xtbson
from xtbson import EPOCH_AWARE, RE_TYPE
from xtbson.binary import Binary
from xtbson.code import Code
from xtbson.dbref import DBRef
from xtbson.max_key import MaxKey
from xtbson.min_key import MinKey
from xtbson.objectid import ObjectId
from xtbson.timestamp import Timestamp

from xtbson.py3compat import PY3, binary_type, string_types
...
def dumps(obj, *args, **kwargs):
    ...
    :class:`~xtbson.binary.Binary` and :class:`~xtbson.code.Code`.
    """
    if not json_lib:
...
def object_hook(dct):
    ...
    if xtbson.has_uuid() and "$uuid" in dct:
        return xtbson.uuid.UUID(dct["$uuid"])
    return dct
...
def default(obj):
    ...
    if isinstance(obj, datetime.datetime):
        # TODO share this code w/ xtbson.py?
    ...
    if PY3 and isinstance(obj, binary_type):
        return {'$binary': base64.b64encode(obj).decode(),
                '$type': "00"}
    if xtbson.has_uuid() and isinstance(obj, xtbson.uuid.UUID):
        return {"$uuid": obj.hex}
