...
class SON(dict):
    ...
    None                                 null           both
    bool                                 boolean        both
    int [#int]_                          int32 / int64  py -> xtbson
    long                                 int64          both
    float                                number (real)  both
    string                               string         py -> xtbson
    unicode                              string         both
    list                                 array          both
    dict / `SON`                         object         both
    datetime.datetime [#dt]_ [#dt2]_     date           both
    compiled re                          regex          both
    `xtbson.binary.Binary`                 binary         both
    `xtbson.objectid.ObjectId`             oid            both
    `xtbson.dbref.DBRef`                   dbref          both
    None                                 undefined      xtbson -> py
    unicode                              code           xtbson -> py
    `xtbson.code.Code`                     code           py -> xtbson
    unicode                              symbol         xtbson -> py
    bytes (Python 3) [#bytes]_           binary         both
    ===================================  =============  ===================

    Note that to save binary data it must be wrapped as an instance of
    `xtbson.binary.Binary`. Otherwise it will be saved as a BSON string
    ...
    .. [#bytes] The bytes type from Python 3.x is encoded as BSON binary with
       ...
       it will be decoded to an instance of :class:`~xtbson.binary.Binary` with
       subtype 0.
    """
