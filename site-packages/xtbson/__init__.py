...
from xtbson.binary import (Binary, OLD_UUID_SUBTYPE,
                         JAVA_LEGACY, CSHARP_LEGACY)
from xtbson.code import Code
from xtbson.dbref import DBRef
from xtbson.errors import (InvalidBSON,
                         InvalidDocument,
                         InvalidStringData)
from xtbson.max_key import MaxKey
from xtbson.min_key import MinKey
from xtbson.objectid import ObjectId
from xtbson.py3compat import b, binary_type
from xtbson.son import SON, RE_TYPE
from xtbson.timestamp import Timestamp
from xtbson.tz_util import utc


try:
    from xtbson import _cbson
    _use_c = True
except ImportError:
    _use_c = False
...
def _element_to_bson(key, value, check_keys, uuid_subtype):
    ...
    raise InvalidDocument("cannot convert value of type %s to xtbson" %
                          type(value))
...
def is_valid(bson):
    ...
    Raises :class:`TypeError` if `xtbson` is not an instance of
    :class:`str` (:class:`bytes` in python 3). Returns ``True``
    if `xtbson` is valid :class:`BSON`, ``False`` otherwise.

    :Parameters:
      - `xtbson`: the data to be validated
    """
...
class BSON(binary_type):
    ...
    def encode(cls, document, check_keys=False, uuid_subtype=OLD_UUID_SUBTYPE):
        ...
        :class:`~xtbson.errors.InvalidDocument` if `document` cannot be
        converted to :class:`BSON`.

        :Parameters:
          - `document`: mapping type representing a document
          - `check_keys` (optional): check if keys start with '$' or
            contain '.', raising :class:`~xtbson.errors.InvalidDocument` in
            either case
    ...
    def decode(self, as_class=dict,
        ...
        :attr:`xtbson.tz_util.utc`. Otherwise (default), all
